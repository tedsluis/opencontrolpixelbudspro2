...
| `GATT-001` | Full GATT service/characteristic discovery, forced (bond removed first) | User (Hardware) | R, W | 🔵 | The secondary BLE/GATT transport (`ARCHITECTURE.md` §1), distinct from the RFCOMM `libmaestro` channel. Zero-discovery across `CAP-002`–`CAP-004` and the 11:42 `CAP-010` attempt (bond removal alone doesn't force it); achieved for the first time via a fresh third-party GATT client (`CAP-017`, 18:30 session), though a truncated wire-log snaplen leaves the handle↔UUID mapping still open — see `CAP-017-FINDINGS.md` and `PROTOCOL.md` §4.3 Option D. **PROPOSAL, pending maintainer approval — `CAP-014` repeat (2026-08-27):** the snaplen half of `CAP-017`'s blocker is now confirmed fixed (0 truncated frames of 4,663), but the handle↔UUID mapping is **still 🔴 OPEN** — this session reused an already-bonded phone with a cached GATT client, so Android served the `0x0c0X`/`0x0f2X` cluster from its cache instead of re-declaring it on the wire (only the GATT service itself was genuinely re-discovered live). Group W's own actual candidate methods (`pm clear com.android.bluetooth`, or the Pixel 9a) remain untried after 3 Group-W-labeled sessions (`CAP-010`, `CAP-017`, `CAP-014`) — see `CAP-014-FINDINGS.md` §4/§8 for the full root-cause analysis and the precise next step. | `PROTOCOL.md` §4.3 |
