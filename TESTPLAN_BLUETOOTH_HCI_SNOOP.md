...
| `GATT-001` | Full GATT service/characteristic discovery, forced (bond removed first) | User (Hardware) | R, W | 🔵 | The secondary BLE/GATT transport (`ARCHITECTURE.md` §1), distinct from the RFCOMM `libmaestro` channel. Also incidentally observed via the device-detail screen on the Pixel 9a session (§4.2 #3), but that trigger only *may* prompt discovery and doesn't force it. ~~Group R's forced bond removal is the reliable trigger~~ — **correction, 2026-08-14: this is factually wrong.** `CAP-003-FINDINGS.md` §1 found Android's GATT database cache survives bond removal (zero live discovery traffic), and `CAP-004-FINDINGS.md` §6 reconfirmed the same negative result — three for three captures with this goal. **`W` (added 2026-08-14, `CAPTURE_BLUETOOTH_HCI_SNOOP.md`)** is the untried, stronger cache-busting candidate: `pm clear com.android.bluetooth`, or discovery from a phone that has never connected to this device before. **`CAP-010` (2026-08-16, 11:42 session) attempted Group W but did not actually run either of its defined methods** — on-screen procedure was a standard system-Settings forget-and-re-pair on the same Pixel 7a, not `pm clear` or the Pixel 9a (`CAP-010-FINDINGS.md` §1, in the `11-42-31` capture folder) — so the resulting 4th-consecutive zero-discovery result (`CAP-010-FINDINGS.md` §2) extends but does not add new evidence beyond the existing three-for-three finding. **Update 2026-08-16 (second `CAP-010` session, 18:30) — partial success via a third, previously-unidentified method, in the `18-30-12` capture folder.** Driving **nRF Connect for Mobile** — a GATT client app that had never before connected to this specific peripheral — did trigger genuine live discovery (137 `Read By Type`/`Read By Group Type`/`Find Information` frames, vs. zero in every prior attempt; `CAP-010-FINDINGS.md` §2 in that folder), recovering a full 15-service GATT profile via video/screenshots (incl. two previously-undocumented 128-bit UUIDs — "Accessory Non-Owner Service" and an unnamed "Unknown Service" — and confirming a standard Battery Service `0x180F` is present, `PROTOCOL.md` §4.3 Option D). **This does not close the row's own goal**, however: that session's log has a severely truncated (~15-byte) per-packet snaplen, so the actual `Read By Group Type`/`Read By Type` response bytes (handle ranges + UUIDs) are not recoverable from the wire capture itself (`CAP-010-FINDINGS.md` §2/§4b/§4c in that folder) — the handle↔UUID mapping this row exists to resolve is still 🔴 open. `pm clear`/Pixel-9a remain genuinely untried, but are no longer the only known path to discovery — a fresh, never-before-connected GATT client app is now a third, proven one. `CAP-014` (planned, `CAPTURE_BLUETOOTH_HCI_SNOOP.md` §9) now targets fixing the snaplen and an on-screen characteristic drill-down, not `pm clear`/Pixel-9a specifically. | — |
