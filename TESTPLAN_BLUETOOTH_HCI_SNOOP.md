...
| `PAIR-004` | 'Forget' clears a pre-existing BLE association/link-key completely, even one not established via the Pixel Buds app | User (Hardware) | A (repeat) | 🔴 | Added 2026-08-14. `CAP-001-FINDINGS.md` §6 found a BLE link and a still-valid link key both existing *before* the on-screen "Forget" tap and before the case reopened — distinct claim from `PAIR-001`'s "fresh pairing procedure." See `CAPTURE_BLUETOOTH_HCI_SNOOP.md`'s Group-A repeat note. **Update 2026-08-26 (`CAP-013`), VOORSTEL — wacht op goedkeuring maintainer:** this question splits into two — the **primary** claim above (did a BLE link/key exist *before* the clearing action?) remains 🔴 fully OPEN, since `CAP-013`'s logging only started *after* the clearing action and the entire subsequent discovery/case-open/pair-button sequence (`CAP-013-FINDINGS.md` §0). The **secondary** claim — does the re-pairing *following* the clearing action show a fresh SSP handshake (clearing worked for whatever state existed right before logging began) or a reused key — is 🟢 answered: a complete fresh-SSP sequence was observed (`Delete Stored Link Key` → `Negative` Link Key Reply → IO Capability/User Confirmation/Simple Pairing Complete → new Link Key Notification, frames 117–270), no key-reuse path. A further repeat with logging verified to start *before* the clearing action is still needed for the primary claim. **Update 2026-08-27 (`CAP-031`), VOORSTEL — wacht op goedkeuring maintainer:** a third attempt, this time with a genuine narrow per-device "Forget" (screenshot-confirmed) and a live snoop-log file-size-polling check — **still did not capture the pre-clearing-action window**: the log's first frame lands 66s after the Forget tap (`CAP-031-FINDINGS.md` §0). **Primary claim remains 🔴 fully OPEN, untested a third time.** The secondary claim is reconfirmed 🟢 (sixth confirming fresh-SSP instance, frames 598–689, `CAP-031-FINDINGS.md` §2/§7). A fourth attempt is still needed, verifying the snoop log's own *content* freshness (not just file size) immediately before the Forget tap. | `CAP-013-FINDINGS.md` §0/§2/§7, `CAP-031-FINDINGS.md` §0/§2/§7/§8 |
