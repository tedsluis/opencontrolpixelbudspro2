...
| `OBS-002` | App running in the background (battery polling) | App (Auto) | AI (planned) | 🔴 | The Fast Pair spec suggests event-driven updates (on value change), not necessarily active polling from the app. Still to confirm whether the official app also actively polls on top of this. **Dedicated capture scenario now planned, added 2026-09-05:** `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AI — a 15+ minute genuinely idle bracket, specifically to extend `CAP-009-FINDINGS.md` §2's existing HFP-only cadence characterization to the DLCI 0x02/0x04/0x08 cross-channel push `CAP-036-FINDINGS.md` §12.5 found. | — |
| `OBS-004` | Does the app query the Buds' **current settings state** (a read/"give me the current value" direction) before any user-initiated change — on (re)connection, and/or on opening a settings screen (EQ, touch controls)? | App (Auto) | AC, AD (planned) | 🔴 | Added 2026-09-04. Deliberately **distinct from `OBS-001`** (app *launch* after a force-close, Group L — a different trigger, and framed around status/battery rather than settings) and **from `FW-002`** (the firmware/serial *info* screen, Group I). What makes this its own research question: every settings entry in `PROTOCOL.md` §4.2/§4.5 rests exclusively on **write**-direction evidence (a UI tap producing a `Sent` frame, answered only by an `Rcvd` echo of the same shape — "no distinct ACK opcode observed"), and no capture has isolated whether a query exists at all. DLCI 0x04's spec-documented `Get ANC state` (`0x11`, `PROTOCOL.md` §4.1) has likewise never been observed on the wire. Directly load-bearing for `ARCHITECTURE.md` §3.1 (State Reconciliation). **A clean negative is a real result for this row**, not an empty one — see `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AC. **`CAP-036`'s single-sample reconnect finding has since been independently replicated 17 times across 10 captures and promoted to 🟢 FACT** (`DECISIONS.md` ADR-021/ADR-022) — `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AD (`CAP-037`, planned, added 2026-09-05) is the first **purpose-built** repeat of that same question, plus the first within-session dock-state-transition test for ADR-024. | `CAP-036-FINDINGS.md` |
| `OBS-005` | Does a physically-realistic reconnect (Buds actually removed from the case and worn) produce wire traffic beyond what a system-Bluetooth-toggle-only reconnect (`CAP-036`/`CAP-037`) already shows? | User (Hardware) | AE (planned) | 🔴 | Added 2026-09-05. Every reconnect this project has captured on the Pixel 7a with the official app so far was triggered by an OS-level Bluetooth toggle while the Buds sat undisturbed in the open case — not the normal user flow. See `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AE (`CAP-038`, planned). | — |
| `OBS-006` | Does the "Notify ANC state" `Settable-toggles` byte (`DECISIONS.md` ADR-024) still track dock state when a `Set` (ANC tap) and a `Get` (forced reconnect) are directly compared within one session at a fixed, known dock state? | User (App) / App (Auto) | AF (planned) | 🔴 | Added 2026-09-05. ADR-024's dock-state reading was built from samples across different sessions; this is the first same-session, fixed-dock-state test of it. See `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AF (`CAP-039`, planned). | — |
| `OBS-007` | Does DLCI 0x02's connect-time RPC-shaped burst (`CAP-036-FINDINGS.md` §4) vary with non-default EQ/touch-controls settings, suggesting a `libmaestro`-side settings-state read-back on reconnect? | App (Auto) | AH (planned) | 🔴 | Added 2026-09-05. Directly relevant to `ARCHITECTURE.md` §3.1 (State Reconciliation) — a second, independent read-back mechanism alongside DLCI 0x04's confirmed ANC read (ADR-021/ADR-022), if confirmed. See `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AH (`CAP-041`, planned). | — |
...
| `PRIV-001` | Semantic decode, via correlation not guessing, of DLCI 0x08's unmapped zero-length `[Group][Code][00 00]`-shaped `Sent` frames (`05 0c`, `04 02`, `04 04`, `04 11`, `04 13`, `04 15`, `0e 04`) found in `CAP-036-FINDINGS.md` §5, structurally identical to DLCI 0x04's confirmed "Get" pattern but never attributed to any known setting | App/OS (Auto) | AG (planned) | 🔴 | Added 2026-09-05. Per `AGENTS.md` §13.6's zero-creativity rule, these can only be decoded by bracketing a known, independently-verifiable changing value (battery discharge, or deliberately varied dock state) across several reconnects and checking whether any `Rcvd` response tracks it. See `CAPTURE_BLUETOOTH_HCI_SNOOP.md` Group AG (`CAP-040`, planned). | — |
