...
**Deny-mode drop capture.** This question was answered by the capture now classified as `CAP-059` —
see the entry immediately below (moved to `CAP-059-FINDINGS.md` §1) for what preceded the Buds' own
`DISC`/`DM` on DLCI 2/4.

### 2026-09-20 (second entry) — moved to `CAP-059-FINDINGS.md`

This entry covered a single capture (`LOGS-001`, now `CAP-059`) rather than a cross-capture pattern
analysis, which is outside this document's own stated scope (see the file header above). Per
`PROJECT_RULES.md` rule 9a, it has been rewritten in place — not duplicated — as `CAP-059-FINDINGS.md`
under `captures/CAP-059-2026-09-20_17-16-59_17-22-29-Group_AU/` (`ai-sessions/0043`,
