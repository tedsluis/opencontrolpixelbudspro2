...
| Video file       | `recording.mp4` — 83.4s, starts 08:50:57, ends 08:52:20 (wall clock, +0200) |
| Log file         | `btsnoop_hci.log` — 233.9s, 08:50:32.67–08:54:26.57 (wall clock, +0200), 2,663 packets |
...
`FINDINGS.md` §5 for what this cost us.

## Event Timeline

Timestamps are the video's own on-screen wall-clock overlay (±1s, since frames were sampled at
1fps); `btsnoop_hci.log` uses the same wall clock (+0200), so times below are directly
...
| 08:51:02–03 | Tap **Forget** | User (App) | — | No immediately visible log effect; a link key was still available and reused at 08:51:12 (frame 911) — see `FINDINGS.md` §6 open question |
...
| 08:51:12–15 | (not directly visible on screen — app UI mid-transition) | — | `PAIR-001` | RFCOMM channels 0, 1, 2, 4, 5 opened (frames 984–1070, 1334); SDP queries for Audio Sink/AVRCP/HFP/A2DP (frames 902–1231); full HFP AT-command handshake incl. `AT+BIEV=2,100` battery report (frames 1236–1310) — see `FINDINGS.md` §3 |
...
| 08:51:32 | Tap **Transparency** (1st) — ANC row now fully active | User (App) | `ANC-004` | No confidently attributable command frame identified — see `FINDINGS.md` §5 |
| 08:51:36 | Right earbud removed from case | User (Hardware) | `CASE-005` | — |
| 08:51:39 | Tap **Off** | User (App) | `ANC-001` | See `FINDINGS.md` §5 |
| 08:51:43 | Tap **Adaptive** | User (App) | `ANC-003` | See `FINDINGS.md` §5 |
| 08:51:49 | Tap **Transparency** (2nd) | User (App) | `ANC-004` | See `FINDINGS.md` §5 |
| 08:51:54 | Tap **Noise Cancellation** | User (App) | `ANC-002` | See `FINDINGS.md` §5 |
| 08:52:00 | Tap **Off** (2nd) | User (App) | `ANC-001` | See `FINDINGS.md` §5 |
...
- The right-bud-removal and left-bud-removal timestamps were adjusted slightly (08:51:20/36
  ...
  by 1–3s).
- Added explicit `btsnoop_hci.log` frame references for every event where log evidence exists,
