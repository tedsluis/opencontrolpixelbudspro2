# Event Notes: Pixel Buds Pro 2 (`libmaestro` / `libgfps`) — Group W Retry Capture (`CAP-010`, 18:30)

**Status:** Reviewed against `CAP-010-recording.mp4` frame-by-frame (sub-second resolution near
key actions, using the video's burned-in wall-clock overlay) and cross-checked against
`CAP-010-btsnoop_hci.log` via `tshark`. This is a **second, independent `CAP-010` capture**,
...
third-party GATT client against the Buds. See `CAP-010-FINDINGS.md` in *this* folder for the
...
|------------------|----------------------------------------------------|
|    Capture ID    |                      `CAP-010`                     |
|      Group(s)    | W (`GATT-001` — GATT discovery forced via a third-party client, nRF Connect — **not** either of Group W's originally-defined candidate methods (`pm clear`/Pixel 9a); see `CAP-010-FINDINGS.md` §1 for that distinction) |
|       Date       |                     2026-08-16                     |
| Firmware version |  `release_5.203` (not re-confirmed on-the-wire this session — no DLCI 0x08 handshake occurred, since the official Companion App's Message-Stream channel was never opened; see `CAP-010-FINDINGS.md` §4) |
...
| Video file       | `CAP-010-recording.mp4` — 419.6s, 18:30:12–18:37:12 (wall clock, +0200) |
| Log file         | `CAP-010-btsnoop_hci.log` — 559.2s, 1,747 packets, 18:31:32.72–18:40:51.93 (wall clock, +0200) — **log starts ~80s after the video** (Bluetooth-on and nRF Connect launch predate the log's own first captured frame) **and continues ~3.7 min after the video ends** |

**Capture-integrity note (see `CAP-010-FINDINGS.md` §2 for full detail):** `CAP-010-btsnoop_hci.log`
...
`CAP-010-FINDINGS.md` was recovered instead via direct frame-by-frame reading of nRF Connect's own
...
Timestamps are the video's own on-screen wall-clock overlay; `CAP-010-btsnoop_hci.log` uses the
...
| 18:31:32.72 | *(not visible on screen — log-only)* First frame captured in `CAP-010-btsnoop_hci.log`: dense burst of LE Extended Advertising Reports from ~20 nearby BLE devices, incl. `04:00:6e:cf:6e:07` ("Pixel Buds Pro 2 van Ted") from frame 2 onward | — | — | Frames 1–651 |
...
- Added Test-ID tags and specific `CAP-010-btsnoop_hci.log` frame references for every event where
...
- **Most significant addition:** the original draft ended at "18:33:53 user selects pixel buds pro
  ...
  **did achieve genuine live GATT discovery**, recovering a full 15-service profile including two
  previously-undocumented 128-bit UUIDs. See `CAP-010-FINDINGS.md` for the complete analysis.
- **Important caveat surfaced during this pass, not present in the original draft:**
  `CAP-010-btsnoop_hci.log` itself is critically truncated at the ACL-payload level for nearly
