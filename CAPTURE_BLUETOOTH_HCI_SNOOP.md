...
| `CAP-014` | *planned* | Pixel 7a or Pixel 9a | TBD | TBD | TBD | W (repeat, done properly) | `GATT-001` | **Re-attempt of Group W, actually executing its defined method this time** — `adb shell pm clear com.android.bluetooth` (confirm acceptable first: clears all bonded devices on the phone) or a genuinely first-time connection from the Pixel 9a — `CAP-010` used bond-removal-only again and reproduced the same 4-for-4 negative discovery result (`CAP-010-FINDINGS.md` §7 item 1) | — | — | planned |
